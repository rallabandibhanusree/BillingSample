      ...
      if (req.payload.request.body.care_manager !== undefined) {
        ...
        if(req.payload.request.body["role"]){
          if(req.payload.request.body.role == "CAREMANAGER"){
            errormsg = gen_errormsg(errormsg, "care_manager field is not allowed for " + req.payload.request.body.role + " role");
            rejectRequest = true;
          }
        }
      }
      ...
      if (req.payload.request.body.member_partner_id !== undefined) {
        ...
        if(req.payload.request.body["role"]){
          if(req.payload.request.body.role == "MEMBER" ||  req.payload.request.body.role == "CAREMANAGER"){
            errormsg = gen_errormsg(errormsg, "member_partner_id is not allowed for " + req.payload.request.body.role + " role");
            rejectRequest = true;
          }
        }
      }
