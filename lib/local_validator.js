  ...
  validate_val_service_order: function (req) {
    return new promise(function (resolve, reject) {
      ...
      if(top_schema_validation_failed){
        errormsg = gen_errormsg(errormsg, " Keys are not allow to have in our system");
        rejectRequest = true;
      }
  ...
  validate_values: function(req, type) {
    return new promise(function (resolve, reject) {
      ...
      if (req.payload.request.body.phone != undefined && _.isObject(req.payload.request.body.phone) && !_.isEmpty(req.payload.request.body.phone)) {
        ...
        var params_keys = Object.keys(req.payload.request.body.phone);
        for (var i = 0; i < params_keys.length; i++) {
        ...
        if(phone_schema_validation_failed){
          errormsg = gen_errormsg(errormsg, "Phone object only allow to have mobile, home and office keys in it");
          rejectRequest = true;
        }
        if (req.payload.request.body.phone.mobile !== undefined && req.payload.request.body.phone.mobile != "") {
          mobile = validator.trim(req.payload.request.body.phone.mobile.toString());
        ...
        if (req.payload.request.body.phone.home !== undefined && req.payload.request.body.phone.home != "") {
          home = validator.trim(req.payload.request.body.phone.home.toString());
          home = home.replace(/-/g, "");
        ...
        if (req.payload.request.body.phone.office !== undefined && req.payload.request.body.phone.office != "") {
          office = validator.trim(req.payload.request.body.phone.office.toString());
