...
// var endpointName = path.basename(path.resolve(__filename));
...
module.exports = function(req, reply) {
    ...
    function returnResonse(message, return_code){
        ...
        //Capture End of Call using custom module and calculate the difference.
        //apiResponse.endTime(req);
        return response;
    }
        ...
        if (req.payload.request.method === 'POST') {
            ...
            try {
                ws.search('[profile.external_id = "' + req.payload.request.body.external_id + '", __class__ = "user"]').on('success', function (searchData, response) {
                    ...
                    returnResonse("Error searching external_id" + error,404);
                });
            }catch (error) {
    ...
    local_validate.validate_req(req)
        .then(function (result) {
            //Capture start of Api call using custom module
            //apiResponse.startTime(logger.info('The call was made from this endpoint : ' + endpointName));
