...
function ping_logout(req){
    return new Promise(function (resolve, reject) {
        ...
        rest.post(revoke_token_url, options_validate).on('success', function (logout_result, response) {
            ...
            logger.error('Did not return a response within ' + ms + ' ms');
            reject({"message": "Timed out" + ms, "status": 504});
...
module.exports = function(req, reply) {
    ...
    local_validate.validate_req(req).then(function (result) {
        ...
        if(ws !== undefined && ws !== "") {
            logger.info("Logout from Cloudmine");
                ...
                localCache.del([access_token, access_token+"WS", access_token + "cch_id", access_token + "external_id", access_token + "member_external_id"], function( err, count ){
                    if( !err ){
                        ping_logout(req).then(function(logout_result){
                            logger.info("Logout successful from Cloudmine and PING");
                        ...
                        logger.error("Failed to logout from Cloudmine" + err);
                        reject(logout_result_error);
                        return returnResponse(reply, "Failed to logout from Cloudmine", 400);
                    }
                });
            }).on('error', function (error){
                logger.info("Logout Error in Cloudmine" + error);
                reject(logout_result_error);
                return returnResponse(reply, "Failed to logout from Cloudmine" + error, 400);
            });
        } else {
            logger.info("There is nothing in Cloudmine to logout");
            localCache.del([access_token, access_token + "WS", access_token + "cch_id", access_token + "external_id", access_token + "member_external_id"], function (err, count) {
                if (!err) {
                    logger.info("Logout successful In Cloudmine");
                    ping_logout(req).then(function(logout_result){
                        logger.info("Logout successful from Cloudmine and PING");
                        resolve(logout_result);
                        return returnResponse(reply, logout_result.message, logout_result.status);
