...
module.exports = {
    createUserAndProfile: function(ws,req,searchData,reply,access_token) {
        return new Promise(function (resolve, reject) {
            var uuid = uuidV4();
            ...
            local_validate.validate_values(req, "create").then(function (result) {
               ...
               // payload["role"] = [req.payload.params.role];
                if(req.payload.params.role !== null && req.payload.params.role !== undefined && req.payload.params.role !== "") {
                    payload["role"] = [req.payload.params.role];
                }
                payload["cch_id"] = uuid;
                ...
                scim_ldap.createLdap(payload, access_token).then(function (ldap_result) {
                    ...
                    ws.createUser({
                        ...
                        var user_acl = {
                            ...
                            "permissions": ["r", "u", "c", "d"],
                            "partner_id": req.payload.params.partner_id,
                            "meta": "admin"
                        };
                        var readonly_acl = {
                            "members": [],
                                ...
                                ws.updateACL(user_acl).on('success', function (Acldata, response) {
                                    ...
                                    for (var i = 0; i < params_keys.length; i++) {
                                        ...
                                        if (params_keys[i] == "role") {
                                            user_data_obj_update["profile"][params_keys[i]] = [req.payload.params[params_keys[i]]];
                                        } else {
                                            user_data_obj_update["profile"][params_keys[i]] = req.payload.params[params_keys[i]];
                                        }
                                    }
                                    ...
                                    if (req.payload.params["care_manager"] != undefined && req.payload.params["care_manager"] != "") {
                                        custom_acl_module.updateAclForCareManager(ws, req.payload.params.partner_id, req.payload.params["care_manager"], "readonly")
                                    ...
                                    } else if (req.payload.params["member_partner_id"] != undefined && req.payload.params["member_partner_id"] != "") {
                                        custom_acl_module.updateAclForCareGiver(ws, req.payload.params["member_partner_id"], userData.__id__, "readonly")
                                            .then(function (updateAcl_result) {
                                                logger.info("Readonly ACL updated in order to provide access the caremanager");
    ...
    updateProfileAndLdap: function(ws,req,data,reply,access_token){
        return new Promise(function (resolve, reject) {
            local_validate.validate_values(req, "update")
                .then(function (result) {
                    ...
                    var acl_properties_caremanager = ["care_manager"];
                    var acl_properties_caregiver = ["member_partner_id"];
                    ...
                    for (var i = 0; i < params_keys.length; i++) {
                        ...
                        if (typeof data[profile_id]["profile"][params_keys[i]] !== "undefined") {
                            if (_.isObject(data[profile_id]["profile"][params_keys[i]]) && _.isObject(req.payload.params[params_keys[i]])) {
                                if (!_.isEqual(data[profile_id]["profile"][params_keys[i]], req.payload.params[params_keys[i]])) {
                                    user_data_obj_update["profile"][params_keys[i]] = req.payload.params[params_keys[i]];
                                    require_update = true;
                            ...
                            } else if (_.isArray(req.payload.params[params_keys[i]]) && _.isArray(data[profile_id]["profile"][params_keys[i]])) {
                                if (check_equal(req.payload.params[params_keys[i]], data[profile_id]["profile"][params_keys[i]])) {
                                    user_data_obj_update["profile"][params_keys[i]] = req.payload.params[params_keys[i]];
                                    require_update = true;
                                    if (ldap_properties.indexOf(params_keys[i]) > -1) {
                                        ldap_update_required = true;
                            ...
                            } else if (!_.isObject(req.payload.params[params_keys[i]]) && _.isObject(data[profile_id]["profile"][params_keys[i]])) {
                                if (data[profile_id]["profile"][params_keys[i]].indexOf(req.payload.params[params_keys[i]]) <= -1) {
                                    user_data_obj_update["profile"][params_keys[i]] = [req.payload.params[params_keys[i]]];
                                    require_update = true;
                                    if (ldap_properties.indexOf(params_keys[i]) > -1) {
                                        ldap_payload[params_keys[i]] = [req.payload.params[params_keys[i]]];
                                ...
                                if (req.payload.params[params_keys[i]] !== data[profile_id]["profile"][params_keys[i]]) {
                                    if (params_keys[i] == "role") {
                                        user_data_obj_update["profile"][params_keys[i]] = [req.payload.params[params_keys[i]]];
                                    } else {
                                        user_data_obj_update["profile"][params_keys[i]] = req.payload.params[params_keys[i]];
                    ...
                    if (ldap_update_required && require_update && !caremanager_acl_update_required && !caregiver_acl_update_required) {
                        if (typeof data[profile_id]["profile"]["ldap_id"] != "undefined" && data[profile_id]["profile"]["ldap_id"] != "") {
                            scim_ldap.updateLdap(ldap_payload, data[profile_id]["profile"]["ldap_id"], access_token).then(function (ldap_result) {
                                ...
                                reject({"message": "scim ldap update error" + error, "status": 400});
                            });
                        } else {
                        ...
                        if (typeof data[profile_id]["profile"]["ldap_id"] != "undefined" && data[profile_id]["profile"]["ldap_id"] != "") {
                            custom_acl_module.updateAclForCareGiver(ws, req.payload.params["member_partner_id"], data[profile_id]["profile"]["user_id"], "readonly")
                                .then(function (updateAcl_result) {
                        ...
                        custom_acl_module.updateAclForCareManager(ws, req.payload.params.partner_id, req.payload.params["care_manager"], "readonly").then(function (updateAcl_result) {
                            logger.info("readonly acl updated in order to provide access to careManager");
                            updateProfileObjectInCLM(ws, update_obj).then(function (updateprofileresult) {
                                resolve({"message": updateprofileresult.message, "status": updateprofileresult.status});
                            }).catch(function (updateProfileError) {
                        ...
                        custom_acl_module.updateAclForCareGiver(ws, req.payload.params["member_partner_id"], data[profile_id]["profile"]["user_id"], "readonly").then(function (updateAcl_result) {
                            logger.info("readonly acl updated in order to provide access to careGiver");
                            updateProfileObjectInCLM(ws, update_obj).then(function (updateprofileresult) {
                                resolve({"message": updateprofileresult.message, "status": updateprofileresult.status});
                            }).catch(function (updateProfileError) {
