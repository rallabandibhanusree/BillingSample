    ...
    createUserAndProfile: function(ws,req,searchData,access_token) {
        return new Promise(function (resolve, reject) {
            ...
            local_validate.validate_values(req, "create").then(function (result) {
                ...
                payload["partner_id"] = req.payload.request.body.partner_id;
                payload["external_id"] = req.payload.request.body.partner_id;
                scim_ldap.createLdap(payload, access_token).then(function (ldap_result) {
                    ...
                    ws.createUser({
                        ...
                        var user_acl = {
                            ...
                            "permissions": ["r", "u", "c", "d"],
                            "partner_id": req.payload.request.body.partner_id,
                            "external_id": req.payload.request.body.partner_id,
                            "meta": "admin"
                        };
                        var readonly_acl = {
                            "members": [],
                                ...
                                ws.updateACL(user_acl).on('success', function (Acldata, response) {
                                    ...
                                    for (var i = 0; i < params_keys.length; i++) {
                                        ...
                                        if (params_keys[i] == "role") {
                                            ...
                                            user_data_obj_update["profile"][params_keys[i]] = req.payload.request.body[params_keys[i]];
                                            if(params_keys[i] == "partner_id") {
                                                user_data_obj_update["profile"]["external_id"] = req.payload.request.body[params_keys[i]];
                                            }
                                        }
                                    }
                                    ...
                                    if (req.payload.request.body["care_manager"] != undefined && req.payload.request.body["care_manager"] != "") {
                                        custom_acl_module.updateAclForCareManager(ws, req.payload.request.body.partner_id, req.payload.request.body["care_manager"], "readonly")
                                    ...
                                    } else if (req.payload.request.body["member_partner_id"] != undefined && req.payload.request.body["member_partner_id"] != "") {
                                        custom_acl_module.updateAclForCareGiver(ws, req.payload.request.body["member_partner_id"], userData.__id__, "readonly")
                                            .then(function (updateAcl_result) {
                                                logger.info("Readonly ACL updated in order to provide access the caremanager");
            ...
            local_validate.validate_values(req, "update")
                .then(function (result) {
                                ...
                                if (req.payload.request.body[params_keys[i]] !== data[profile_id]["profile"][params_keys[i]]) {
                                    ...
                                    if (ldap_properties.indexOf(params_keys[i]) > -1) {
                                        if (params_keys[i] == "role") {
                                            ...
                                            ldap_payload[params_keys[i]] = req.payload.request.body[params_keys[i]];
                                            if(params_keys[i] == "partner_id"){
                                                ldap_payload["external_id"] = req.payload.request.body[params_keys[i]];
                                            }
                                        }
                                        ldap_update_required = true;
                                    }
                    ...
                    if (ldap_update_required && require_update && !caremanager_acl_update_required && !caregiver_acl_update_required) {
                        ...
                        if (typeof data[profile_id]["profile"]["cch_id"] != "undefined" && data[profile_id]["profile"]["cch_id"] != "") {
                            custom_acl_module.updateAclForCareGiver(ws, req.payload.request.body["member_partner_id"], data[profile_id]["profile"]["user_id"], "readonly")
                                .then(function (updateAcl_result) {
                        ...
                        custom_acl_module.updateAclForCareManager(ws, req.payload.request.body.partner_id, req.payload.request.body["care_manager"], "readonly").then(function (updateAcl_result) {
                            logger.info("readonly acl updated in order to provide access to careManager");
                            updateProfileObjectInCLM(ws, update_obj).then(function (updateprofileresult) {
                                resolve({"message": updateprofileresult.message, "status": updateprofileresult.status});
                            }).catch(function (updateProfileError) {
                        ...
                        custom_acl_module.updateAclForCareGiver(ws, req.payload.request.body["member_partner_id"], data[profile_id]["profile"]["user_id"], "readonly").then(function (updateAcl_result) {
                            logger.info("readonly acl updated in order to provide access to careGiver");
                            updateProfileObjectInCLM(ws, update_obj).then(function (updateprofileresult) {
                                resolve({"message": updateprofileresult.message, "status": updateprofileresult.status});
                            }).catch(function (updateProfileError) {
